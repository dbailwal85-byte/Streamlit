...
# =============================================================================
# 4. HELPER TO CREATE SAFE PAGE OBJECTS
# =============================================================================
def get_safe_page(file_path: str, title: str, icon: str):
    """
    Checks if a page file exists on disk. If found, returns the st.Page;
    otherwise, returns a fallback page warning the user to create the file.
    """
    if os.path.exists(file_path):
        ...
        def placeholder_func():
            ...
            st.warning(
                f"⚠️ Page module file `{file_path}` was not found. "
                f"Please generate and save `{file_path}` in your repository."
            )

        return st.Page(placeholder_func, title=title, icon=icon)
...
# =============================================================================
def main() -> None:
    """Configures page navigation and sidebar dataset status."""
    st.sidebar.markdown(
        """
    ...
    pages = {
        "Main": [
            ...
            get_safe_page("pages/01_Data_Upload.py", title="Data Upload", icon="📤"),
        ],
        "Pipeline": [
            get_safe_page("pages/02_Data_Understanding.py", title="Data Understanding", icon="🔍"),
            get_safe_page("pages/03_Data_Cleaning.py", title="Data Cleaning", icon="🧹"),
            get_safe_page("pages/04_Exploratory_Data_Analysis.py", title="Exploratory Data Analysis", icon="📊"),
        ],
        "Analytics": [
            get_safe_page("pages/05_Business_Dashboard.py", title="Business Dashboard", icon="📈"),
            get_safe_page("pages/06_Customer_Analytics.py", title="Customer Analytics", icon="👥"),
            get_safe_page("pages/07_Product_Analytics.py", title="Product Analytics", icon="📦"),
            get_safe_page("pages/08_Sales_Analytics.py", title="Sales Analytics", icon="💰"),
        ],
        "Advanced Tools": [
            get_safe_page("pages/09_SQL_Database.py", title="SQL Database", icon="🗄️"),
            get_safe_page("pages/10_Sales_Forecasting.py", title="Sales Forecasting", icon="🤖"),
            get_safe_page("pages/11_Reports.py", title="Reports Generator", icon="📄"),
        ],
    }
